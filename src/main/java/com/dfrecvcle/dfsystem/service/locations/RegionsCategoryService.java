...
@Service
public class RegionsCategoryService {

    private Logger logger = LoggerFactory.getLogger(RegionsCategoryService.class);

    @Resource
    private RegionsCategoryMapper regionsCategoryMapper;
    ...
    public void insert(RegionsCategory category, HttpServletRequest request) {
        ...
        int count = regionsCategoryMapper.isExistCategoryName(null, category.getName());
        if(count > 0) {
...
//            category.setUpdatedBy(null);
            result = regionsCategoryMapper.insertArticleCategory(category);
    ...
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public void update(RegionsCategory category, HttpServletRequest request) {
        ...
        // 分類名稱已存在
        if(category.getName() != null && !category.getName().isEmpty()) {
            int count = regionsCategoryMapper.isExistCategoryName(category.getId(), category.getName());
            if(count > 0) {
                throw new BusinessRunTimeException(ExceptionConstants.ARTICLE_CATEGORY_IS_EXIST_CODE,
                        String.format(ExceptionConstants.ARTICLE_CATEGORY_IS_EXIST_MSG, category.getName()));
            }
        ...
        try{
            ...
            result = regionsCategoryMapper.updateByPrimaryKeySelective(category);
    ...
    public void deleteCategory(Integer id, HttpServletRequest request) {
        RegionsCategory tag = regionsCategoryMapper.selectByPrimaryKey(id);
        if(tag == null) {
            throw new BusinessRunTimeException(ExceptionConstants.ARTICLE_CATEGORY_EDIT_FAILED_CODE,
                    ExceptionConstants.ARTICLE_CATEGORY_EDIT_FAILED_MSG);
        }
        try {
            ...
            regionsCategoryMapper.updateByPrimaryKeySelective(tag);
    ...
    public List<JSONObject> getAllArticleCategory() {
        List<RegionsCategory> list = regionsCategoryMapper.getListAll();
...
//            json.put("code", tag.getCode());
    ...
    public List<RegionsCategory4Article> getArticleCategoryList(String keyword, String start, String end, String isDelete,
                                                                Integer currentPage, Integer pageSize) {
        return regionsCategoryMapper.getListByExample(keyword, start, end, isDelete, currentPage*pageSize, pageSize);
    }

    public int getArticleCategoryCount(String keyword, String start, String end, String isDelete) {
        return regionsCategoryMapper.getListByExampleCount(keyword, start, end, isDelete);
    }

    public RegionsCategory getCategory(Integer id) {
        RegionsCategory result = null;
        try{
            result = regionsCategoryMapper.selectByPrimaryKey(id);
        }catch(Exception e){
