...
import com.dfrecvcle.dfsystem.live.mappers.LocationsBackendMapper;
...
@Service
public class LocationsBackendService {

    private Logger logger = LoggerFactory.getLogger(LocationsBackendService.class);

//    @Resource
////    private ArticleMapper articleMapper;

    @Resource
    private LocationsBackendMapper locationsBackendMapper;
    ...
    public void insert(LocationsBackend article, HttpServletRequest request) {
        ...
        int count = locationsBackendMapper.isExistTitle(null, article.getTitle());
        if(count > 0) {
...
//            }
            ...
            result = locationsBackendMapper.insertArticle(article);
    ...
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public void update(LocationsBackend article, HttpServletRequest request) {
        ...
        // 分類名稱已存在
        if(article.getTitle() != null && !article.getTitle().isEmpty()) {
            int count = locationsBackendMapper.isExistTitle(article.getId(), article.getTitle());
            if(count > 0) {
                throw new BusinessRunTimeException(ExceptionConstants.ARTICLE_TITLE_IS_EXIST_CODE,
                        String.format(ExceptionConstants.ARTICLE_TITLE_IS_EXIST_MSG, article.getTitle()));
            }
        ...
        try{
            ...
            result = locationsBackendMapper.updateByPrimaryKeySelective(article);

            logService.insertLog("據點",
    ...
    public void updateBrowserCount(Long id, Integer count, HttpServletRequest request) {
        try {
            LocationsBackend article = locationsBackendMapper.selectByPrimaryKey(id);
            if(article == null) {
...
//            article.setBrowserCount(article.getBrowserCount()+count);
            ...
            locationsBackendMapper.updateByPrimaryKeySelective(article);
    ...
    public List<LocationsBackend> getArticleList(String keyword, List<Integer> category, List<String> filter, Long hotTag,
                                                 String start, String end, Integer currentPage, Integer pageSize,List<Integer>regions) {
        Integer latest = null;
...
//                }
        ...
        return locationsBackendMapper.getListByExample(keyword, category, hotTag, latest, featured, publish, top, delete,
                start, end, currentPage*pageSize, pageSize,regions);
    }

    public int getArticleCount(String keyword, List<Integer> category, List<String> filter, Long hotTag,
                               String start, String end, List<Integer> regions) {
        Integer latest = null;
        Integer featured = null;
        Integer publish = null;
        Integer top = null;
...
//                }
        ...
        return locationsBackendMapper.getListByExampleCount(keyword, category, hotTag, latest, featured, publish, top, delete, start, end,regions);
    }

    public LocationsBackend getArticle(Long id) {
        LocationsBackend result = null;
        try{
            result = locationsBackendMapper.selectByPrimaryKey(id);
        }catch(Exception e){
