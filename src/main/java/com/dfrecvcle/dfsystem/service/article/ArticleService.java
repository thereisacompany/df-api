package com.dfrecvcle.dfsystem.service.article;
...
public class ArticleService {
    ...
    public void insert(Article article, HttpServletRequest request) {
        ...
        int count = articleMapper.isExistTitle(null, article.getTitle());
        if(count > 0) {
    ...
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public void update(Article article, HttpServletRequest request) {
        ...
        // 分類名稱已存在
        if(article.getTitle() != null && !article.getTitle().isEmpty()) {
            int count = articleMapper.isExistTitle(article.getId(), article.getTitle());
            if(count > 0) {
                throw new BusinessRunTimeException(ExceptionConstants.ARTICLE_TITLE_IS_EXIST_CODE,
                        String.format(ExceptionConstants.ARTICLE_TITLE_IS_EXIST_MSG, article.getTitle()));
            }
    ...
    public List<Article> getArticleList(String keyword, List<Integer> category, List<String> filter, Long hotTag,
        ...
        return articleMapper.getListByExample(keyword, category, hotTag, latest, featured, publish, top, delete,
    ...
    public int getArticleCount(String keyword, List<Integer> category, List<String> filter, Long hotTag,
        ...
        return articleMapper.getListByExampleCount(keyword, category, hotTag, latest, featured, publish, top, delete, start, end);
    }

    public Article getArticle(Long id) {
        Article result = null;
        try{
            result = articleMapper.selectByPrimaryKey(id);
        }catch(Exception e){
