package com.dfrecvcle.dfsystem.service.articlecategory;
...
public class ArticleCategoryService {
    ...
    private Logger logger = LoggerFactory.getLogger(ArticleCategoryService.class);
    ...
    public void insert(ArticleCategory category, HttpServletRequest request) {
        ...
        int count = articleCategoryMapper.isExistCategoryName(null, category.getName());
        if(count > 0) {
        ...
        try{
            ...
            result = articleCategoryMapper.insertArticleCategory(category);
    ...
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public void update(ArticleCategory category, HttpServletRequest request) {
        ...
        // 分類名稱已存在
        if(category.getName() != null && !category.getName().isEmpty()) {
            int count = articleCategoryMapper.isExistCategoryName(category.getId(), category.getName());
            if(count > 0) {
                throw new BusinessRunTimeException(ExceptionConstants.ARTICLE_CATEGORY_IS_EXIST_CODE,
                        String.format(ExceptionConstants.ARTICLE_CATEGORY_IS_EXIST_MSG, category.getName()));
            }
        ...
        try{
            ...
            result = articleCategoryMapper.updateByPrimaryKeySelective(category);
    ...
    public void deleteCategory(Integer id, HttpServletRequest request) {
        ArticleCategory tag = articleCategoryMapper.selectByPrimaryKey(id);
        if(tag == null) {
            throw new BusinessRunTimeException(ExceptionConstants.ARTICLE_CATEGORY_EDIT_FAILED_CODE,
                    ExceptionConstants.ARTICLE_CATEGORY_EDIT_FAILED_MSG);
        }
        try {
            ...
            articleCategoryMapper.updateByPrimaryKeySelective(tag);
    ...
    public List<JSONObject> getAllArticleCategory() {
        List<ArticleCategory> list = articleCategoryMapper.getListAll();
...
//            json.put("code", tag.getCode());
    ...
    public List<ArticleCategory4Article> getArticleCategoryList(String keyword, String start, String end, String isDelete,
                                                                Integer currentPage, Integer pageSize) {
        return articleCategoryMapper.getListByExample(keyword, start, end, isDelete, currentPage*pageSize, pageSize);
    }

    public int getArticleCategoryCount(String keyword, String start, String end, String isDelete) {
        return articleCategoryMapper.getListByExampleCount(keyword, start, end, isDelete);
    }

    public ArticleCategory getCategory(Integer id) {
        ArticleCategory result = null;
        try{
            result = articleCategoryMapper.selectByPrimaryKey(id);
        }catch(Exception e){
