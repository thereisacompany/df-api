package com.dfrecvcle.dfsystem.filter;
...
@WebFilter(filterName = "LogCostFilter", urlPatterns = {"/*"},
        initParams = {@WebInitParam(name = "ignoredUrl", value = ".ico"),
                      @WebInitParam(name = "filterPath",
                              value = "/user/login#" +
                                      "/api/v1/locations/list#/api/v1/locations/regions#/api/v1/locations/business#" +
                                      "/api/v1/milestones/list#/api/vi/milestones/listyear#" +
                                      "/api/v1/news/#/feedback/randomImage#" +
                                      "/api/v1/news/list#/feedback/addFeedback#" +
                                      "/other/category/getallList#" +
                                      "/v2/api-docs#/webjars#/swagger" )})
public class LogCostFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        //具體，比如：處理若用户未登入，則跳轉到登入頁
        Object userId = redisService.getObjectFromSessionByKey(servletRequest,"userId");
        if(userId!=null) { //如果已登入，不阻止
            chain.doFilter(request, response);
            return;
        }
        if (requestUrl != null && (requestUrl.contains("/swagger") ||
        ...
        if(requestUrl != null && !requestUrl.contains("/user/logout") && !requestUrl.contains("/function/findMenuByPNumber")) {
            ...
            res.put("code", 200);
            res.put("message", "loginOut");
            res.put("type", "");

            servletResponse.getWriter().write(res.toJSONString());
        }
    }
